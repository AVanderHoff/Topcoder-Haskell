...
breakKeys :: [Int] -> [[Int]]
breakKeys [] = []
breakKeys (x:xs) = [1 .. x]:breakKeys xs

tomekPhone xs ys = if ((sum ys) < (length xs)) then (-1) else (sum $ zipWith (*) rs qs)
   where ws =  concat $ transpose $ breakKeys ys
         qs =  reverse $ sort xs 
         rs = take (length xs) ws 



--  Problem Statement for TomekPhone
...
--        Single Round Match 560 Round 1 - Division II, Level Two