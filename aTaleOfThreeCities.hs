...
threeCities ax ay bx by cx cy = sum compareCitiesList
	where a = zip ax ay
              b = zip bx by
              c = zip cx cy
              distance (m,n) (o,p) = sqrt (fromIntegral(((m-o)^2) + ((n-p)^2))) 
	      compareCities w x = minimum [ (distance e f) | e <- w , f <- x ]
	      compareCitiesList = (take 2) $ sort [ (compareCities a b) ,(compareCities a c) , (compareCities b c)]
