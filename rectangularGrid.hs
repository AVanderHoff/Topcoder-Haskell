rectangularGrid x y = ((combTwoPlus1 x)*(combTwoPlus1 y)) - squaresTotal - (x*y)
    where combTwoPlus1 a = (a*(a+1)) `div` 2
          squareSide m n = m - n + 1
	  squaresTuples = map (\w -> ((squareSide x w),(squareSide y w))) [2 .. (min x y)]
	  squaresTotal = sum $ map (\(c,d) -> (c*d)) squaresTuples





-- 	  TopCoder problem "RectangularGrid" used in SRM 146 (Division I Level One , Division II Level Two)
